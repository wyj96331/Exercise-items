        ...
        <div class="bread">
          ...
          <ul class="fl sui-tag">
            ...
            <li class="with-x" v-if="searchData.trademark">
              ...
              }}<i @click="removeRrademark">×</i>
            </li>
            <!-- 平台售卖属性的面包屑 -->
            <li
              class="with-x"
              v-for="(attrValue, index) in searchData.props"
              :key="index"
            >
...
export default {
  ...
  mounted () {
    this.getSearchData()
    this.searchData.order.indexOf('1')
  },
  components: {
    SearchSelector
  },
  methods: {
    ...
    // 删除分类名字
    removeCategoryName () {
      // 把相应的字段变成undefined，当前字段不会发送给服务器，
    ...
    getAttrInfo (attr, item) {
      const props = `${attr.attrId}:${item}:${attr.attrName}`
      if (this.searchData.props.indexOf(props) === -1) this.searchData.props.push(props)
      this.getSearchData()
    },
    // 删除平台售卖属性面包屑
    removeAttr (index) {
    ...
    changeOrder (flag) {
      ...
      const originFlag = this.searchData.order.split(':')[0]
      const originSort = this.searchData.order.split(':')[1]
      let newOrder = ''
      if (flag === originFlag) {
